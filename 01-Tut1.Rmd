...
# ---------- Graph 1 ----------
png("_book/figures/conditional_coffee_given_sleep.png",
    width = 800, height = 500)

cond_coffee_sleep <- matrix(
  c(61.29, 38.71,
    32.76, 67.24),
  nrow = 2, byrow = TRUE
)

rownames(cond_coffee_sleep) <- c("7 or less hours", "More than 7 hours")
colnames(cond_coffee_sleep) <- c("Light", "Heavy")
...
bp <- barplot(
  ...
  ylab = "Percentage",
  xlab = "Level of sleep",
  main = "Coffee consumption given level of sleep",
  las = 1
)

for (i in seq_len(ncol(cond_coffee_sleep))) {
  cum <- 0
  for (j in seq_len(nrow(cond_coffee_sleep))) {
    v <- cond_coffee_sleep[i, j]
...
legend("bottom", inset=-0.25,
       legend=c("Light coffee", "Heavy coffee"),
...
# ---------- Graph 2 ----------
png("_book/figures/conditional_sleep_given_coffee.png",
    width = 800, height = 500)

cond_sleep_coffee <- matrix(
  c(75, 25,
    48, 52),
  nrow = 2, byrow = TRUE
)

rownames(cond_sleep_coffee) <- c("Light coffee", "Heavy coffee")
colnames(cond_sleep_coffee) <- c("7 or less hours", "More than 7 hours")
...
bp2 <- barplot(
  ...
  ylim = c(0, 100),
  ylab = "Percentage",
  xlab = "Level of coffee consumed",
  main = "Level of sleep given coffee consumption",
  las = 1
)

for (i in seq_len(ncol(cond_sleep_coffee))) {
  cum <- 0
  for (j in seq_len(nrow(cond_sleep_coffee))) {
    v <- cond_sleep_coffee[i, j]
...
legend("bottom", inset=-0.25,
       legend=c("7 or less hours", "More than 7 hours"),
