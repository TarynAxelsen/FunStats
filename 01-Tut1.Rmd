...
```{=html}
<style>
  .box { max-width:750px; margin:0; padding:0; }
  .note { font-size:.9rem; color:#555; margin:.25rem 0 .75rem; }
  table.simp { width:100%; border-collapse:collapse; margin-top:6px; }
  table.simp th, table.simp td { border:1px solid #ccc; padding:8px; text-align:center; }
  table.simp th { background:#f7f7f7; }
  table.simp input { width:90px; padding:4px; text-align:center; }
...
<div class="box" id="cgs-box">
  <p class="h5">(i) Coffee consumed <em>given</em> level of sleep (percentages)</p>
  <div class="note">
    Enter Light/Heavy as <strong>row percentages</strong> within each sleep group (each row should sum to 100%).
  </div>

  <table class="simp" aria-label="Coffee given Sleep">
    <tr>
      <th>Sleep group</th>
      <th>Light</th>
      <th>Heavy</th>
    ...
    <tr>
      ...
      <td><input class="cgs-in" data-answer="100" placeholder="%"></td>
    </tr>
    <tr>
      <th>More than 7 hours</th>
  ...
  <button class="btn" id="cgs-check">Check</button>
  <button class="btn" id="cgs-show">Show answers</button>
  <button class="btn" id="cgs-reset">Reset</button>
  <button class="btn" id="cgs-hint">Hint</button>
  <div id="cgs-result"></div>

  <div class="hint" id="cgs-hintbox">
    Use <span class="mono">(cell count) / (sleep group total) × 100</span>.
    <ul style="margin:.4rem 0;">
      <li>Within ≤7h: Light = <span class="mono">57 / 93 × 100 = 61.29%</span>; Heavy = <span class="mono">36 / 93 × 100 = 38.71%</span></li>
      <li>Within &gt;7h: Light = <span class="mono">19 / 58 × 100 = 32.76%</span>; Heavy = <span class="mono">39 / 58 × 100 = 67.24%</span></li>
    </ul>
    Your answer is accepted to <strong>1 or 2 decimal places </strong> (e.g., <span class="mono">61.3</span> or <span class="mono">61.29</span>).
  </div>
</div>

<script>
(function(){
  const tol2 = 0.005; // numeric closeness (~2dp)
  const r1 = x => Math.round(x*10)/10;
  const r2 = x => Math.round(x*100)/100;
  const ok = (u, k) => isFinite(u) && (Math.abs(u-k)<tol2 || Math.abs(r1(u)-r1(k))<1e-6 || Math.abs(r2(u)-r2(k))<1e-6);

  const ins = () => document.querySelectorAll(".cgs-in");
  const out = document.getElementById("cgs-result");
  document.getElementById("cgs-check").onclick = () => {
    let c=0,t=0; ins().forEach(el=>{ t++; const u=Number(el.value), k=Number(el.dataset.answer);
      el.classList.remove("correct","incorrect"); if(ok(u,k)){el.classList.add("correct"); c++;} else {el.classList.add("incorrect");}});
  ...
  const hb=document.getElementById("cgs-hintbox"); document.getElementById("cgs-hint").onclick=()=>{ hb.style.display = (hb.style.display==="block")?"none":"block"; };
})();
</script>
```
...
<div class="box" id="sgc-box">
  <p class="h5">(ii) Level of sleep <em>given</em> coffee consumed (percentages)</p>
  <div class="note">
    Enter ≤7h / &gt;7h as <strong>row percentages</strong> within each coffee group (each row should sum to 100%).
  </div>

  <table class="simp" aria-label="Sleep given Coffee">
    <tr>
      <th>Coffee group</th>
      <th>7 or less hours</th>
      <th>More than 7 hours</th>
    ...
    <tr>
      ...
      <td><input class="sgc-in" data-answer="100" placeholder="%"></td>
    </tr>
    <tr>
      <th>Heavy</th>
  ...
  <div class="hint" id="sgc-hintbox">
    Use <span class="mono">(cell count) / (coffee group total) × 100</span>.
    <ul style="margin:.4rem 0;">
    ...
    Answers accepted to <strong>1 or 2 d.p.</strong>
  </div>
</div>

<script>
(function(){
  const tol2 = 0.005, r1=x=>Math.round(x*10)/10, r2=x=>Math.round(x*100)/100;
  const ok=(u,k)=>isFinite(u)&&(Math.abs(u-k)<tol2||Math.abs(r1(u)-r1(k))<1e-6||Math.abs(r2(u)-r2(k))<1e-6);

  const ins=()=>document.querySelectorAll(".sgc-in");
  const out=document.getElementById("sgc-result");
  document.getElementById("sgc-check").onclick=()=>{let c=0,t=0; ins().forEach(el=>{t++;const u=Number(el.value),k=Number(el.dataset.answer);
    el.classList.remove("correct","incorrect"); if(ok(u,k)){el.classList.add("correct");c++;} else {el.classList.add("incorrect");}});
    out.textContent=`Score: ${c} of ${t}`;};
  ...
  const hb=document.getElementById("sgc-hintbox"); document.getElementById("sgc-hint").onclick=()=>{hb.style.display=(hb.style.display==="block")?"none":"block";};
})();
</script>
```
### Q2(g){-}
