...
level of sleep, the percentages differ noticeably between the sleep categories. For
example, a much larger proportion of students who sleep more than 7 hours are heavy
coffee drinkers compared to those who sleep 7 hours or less. Because the distribution
of coffee consumption changes depending on the level of sleep, this indicates an
...
<div class="cl-wrap" id="m1-software-checklist">
  <div class="cl-card">
    ...
    <div class="cl-note">
      On successful completion of this module, you should be able to use statistical software to complete the following tasks. Tick each outcome once you have
      successfully demonstrated it.
    </div>

    <div class="cl-prog">
      <div class="cl-bar" id="cl-bar-soft"></div>
    </div>
    <div class="cl-stats" id="cl-stats-soft">0 of 0 completed</div>

    <div class="cl-controls">
      <button class="btn" id="cl-soft-check-all">Mark all done</button>
      <button class="btn" id="cl-soft-clear-all">Clear all</button>
      <span class="cl-note">(* denotes expanded competencies)</span>
    </div>
  </div>
...
(function(){
  ...
  const boxes = Array.from(root.querySelectorAll("input[type=checkbox][data-id]"));
  const bar = document.getElementById("cl-bar-soft");
  const stats = document.getElementById("cl-stats-soft");
  const btnAll = document.getElementById("cl-soft-check-all");
  const btnClear = document.getElementById("cl-soft-clear-all");
